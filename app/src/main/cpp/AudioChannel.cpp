...
void AudioChannel::audio_decode() {
    AVPacket * packet = 0;
    while (isPlaying) {
        LOGD("audio解码队列：循环获取解码的数据, 队列长度：%d", packetQueue.queueSize());
        // 生产快  消费慢
...
void bqPlayerCallback(SLAndroidSimpleBufferQueueItf bq, void *context) {
    // PCM 在 frameQueue 队列中，所以和视频一样，也要这些操作才行
...
int AudioChannel::getPCM() {
    ...
    // PCM 在 frameQueue 队列中
    AVFrame * frame = 0;
    while(isPlaying) {
        LOGD("audio-播放队列：循环队列中获取帧数据, 队列长度：%d", frameQueue.queueSize());
